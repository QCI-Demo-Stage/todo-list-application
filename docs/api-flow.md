...
High-level routing for the Todo REST service defined in [`api-spec.yaml`](../api-spec.yaml). The Node.js server runs on port **8000** by default. When `NODE_ENV` is **staging** or **development**, the OpenAPI spec and Swagger UI are served; in **production**, documentation routes return 404 while `/health` remains available.
